...
int OBD2Class::pidRead(uint8_t mode, uint8_t pid, void* data, int length)
{
  ...
  for (int retries = 10; retries > 0; retries--) {
    ...
    */
    client.write(0x02); // number of additional bytes
    client.write(mode);
    client.write(pid);
  }
  ...
  for (unsigned long start = millis(); (millis() - start) < _responseTimeout;) {
      if ((splitResponse ? (client.read() == 0x10 && client.read()) : client.read()) &&
          (client.read() == (mode | 0x40) && client.read() == pid))
      {
          ...
          if (!splitResponse)
          {
              return client.readBytes((uint8_t *)data, length);
          }

          int read = client.readBytes((uint8_t *)data, 3);
          ...
          for (int i = 0; read < length; i++)
          {
              ...
              if (_useExtendedAddressing)
              {
                  //client.beginExtendedPacket(0x18db33f1, 8);
              }
              else
              {
                  //client.beginPacket(0x7df, 8);
              }
              client.write(0x30);
              //client.endPacket();

              // wait for response
              while (client.read() != (0x21 + i))
                  ; // correct sequence number

              while (client.available())
              {
                  ((uint8_t *)data)[read++] = client.read();
              }
          }

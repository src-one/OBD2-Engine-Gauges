...
byte Elm327::begin()
{
    ...
    */
    /*
    uint8_t address[6]  = {0x00, 0x1D, 0xA5, 0x1B, 0x4E, 0xCA};
    String name = "V-LINK";
    char *pin = "1234";
...
*/
    ...
    runCommand("AT ST 32", data, 20);
    //runCommand("AT ST 90", data, 20);
...
{
#if ELM_WIFI
    Serial.println("Attempting to connect to ELM327 vie Wlan...");
...
#else
    Serial.println("Attempting to connect to ELM327 via Bluetooth...");
    ...
    if (!client.connect(address))
    {
        Serial.println("Couldn't connect to OBD - Phase 1");
        while (1)
            ;
    }
#endif
...
byte Elm327::oilTemperature(int &temp)
{
    ...
    status = getBytes("01", "41", "5C", values, 1);
...
byte Elm327::driverDemandEngineTorque(int &torque)
{
    byte status;
    byte values[1];
...
byte Elm327::actualEngineTorque(int &torque)
{
    byte status;
    byte values[1];
...
byte Elm327::getBytes(const char *mode, const char *chkMode, const char *pid, byte *values, unsigned int numValues)
{
    ...
    if (data[0] != chkMode[0] or data[1] != chkMode[1] or data[3] != pid[0] or data[4] != pid[1])
    {
        return ELM_GARBAGE;
    }
...
byte Elm327::runCommand(const char *cmd, char *data, unsigned int dataLength)
{
    ...
    flush();
    client.write((const uint8_t *)cmd, sizeof(cmd) + 1);
    //client.write(cmd);
    client.write('\r');

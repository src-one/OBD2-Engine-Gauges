...
public:
    ...
    //uint8_t address[6]  = {0x00, 0x1D, 0xA5, 0x1B, 0x4E, 0xCA}; // V-Link
    //uint8_t address[6] = {0x00, 0x1D, 0xA5, 0x04, 0x49, 0x8E}; //  Blau
    //uint8_t address[6] = {0xC4, 0x4C, 0x0C, 0x08, 0x95, 0xAA}; // XR7
    //uint8_t address[6] = {0x00, 0x00, 0x00, 0x33, 0x33, 0x33}; //  ELM327 small
    uint8_t address[6] = {0xAA, 0xBB, 0xCC, 0x11, 0x22, 0x33}; //  ELM327 white
    char *pin = "1234";
    ...
    /**
		 * Gets the Oil Temperature.  Reads PID 5C from the OBD interface and sets
		 * temp to the value returned after conversion.
		 * @param[out]	temp	Signed integer value is set to the oil temperature.
		 * - Minimum: -40
		 * - Maximum Value: 210
		 * - Units: °C
		 */
    ...
    byte driverDemandEngineTorque(int &torque);
    byte actualEngineTorque(int &torque);
